...
class Scriptpython:
    ...
    def lancer1(self):
        ...
        if matermin == "py":
            myprogram="python3"
        wow=self.name.rfind("/")
        monfichier=Fichier("./uploads","lancer_"+self.name.replace("/","").replace(".","")+".sh").ecrire("""xterm  -fa 'Monospace' -fs 20 -l -hold -e "cd {myroot}{myname} && echo \\\"c'est  mon script\\\" && bash -l -c \\\"{program} ./{name}\\\""
